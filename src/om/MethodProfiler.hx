...
/**
	Usage:
		...
		Add build meta to the class you want to profile: @:build(om.MethodProfiler.profile(true))
		For sub classes: @:autoBuild(om.MethodProfiler.profile(true))

		Print results: MethodProfiler.print()

		By default all methods get profiled.
		Set @noProfile on the methods you want to blacklist.
		Or pass false as argument to MethodProfiler.profile and whitelist methods with @profile.
*/
class MethodProfiler {
	...
	*/
	public static function end( className : String, methodName : String ) {
		...
		if( cProfile == null || !cProfile.exists( methodName ) )
			throw 'om.MethodProfiler.end called on a function that was never started [$className.$methodName]';
	...
	*/
	//macro
	public static function profile( allMethods = true ) : Array<Field> {
		...
		for( field in fields ) {
			...
			case FFun(fun):
				...
				if( (!allMethods && !FieldTools.hasMeta( field, 'profile' ) )
				...
				fun.expr = macro {
					om.MethodProfiler.start( $v{clsName}, $v{methodName} );
					${fun.expr};
				};
				...
				if( !lastWasReturn ) {
					fun.expr = macro {
						${fun.expr};
						om.MethodProfiler.end( $v{clsName}, $v{methodName} );
						return;
					}
				}
				//} //TODO?
